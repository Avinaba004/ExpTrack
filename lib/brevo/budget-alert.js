...
export async function maybeSendBudgetBreachNotification({ userId, accountId, transactionDate }) {
  try {
    ...
    console.log(`[BUDGET-ALERT] Total spent: ₹${totalSpent}, Monthly limit: ₹${monthlyLimit}, Last alert sent: ${budget.lastAlertSent}`);

    const shouldNotify = totalSpent >= monthlyLimit && isNewMonth(budget.lastAlertSent, new Date());
    console.log(`[BUDGET-ALERT] Should notify: ${shouldNotify} (totalSpent >= limit: ${totalSpent >= monthlyLimit}, isNewMonth: ${isNewMonth(budget.lastAlertSent, new Date())})`);
    ...
    if (emailResult.success) {
      console.log(`[BUDGET-ALERT] Updating budget.lastAlertSent for budget=${budget.id}`);
